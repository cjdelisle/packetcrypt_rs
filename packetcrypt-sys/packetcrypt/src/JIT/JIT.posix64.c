...
static inline uint32_t rh_branch(int pc, rh_state_compile_t* state) {
  ...
  if(state->nextpc >= state->npc) { // No dynamic label is left, increase the amount
    state->npc *= 2;
...
static inline rh_jit_program_t* rh_link_and_encode(dasm_State** state)
{
  ...
  if (buffer == MAP_FAILED) {
    fprintf(stderr, "Error allocating memory during JIT encoding: %s\n", strerror(errno));
    exit(1);
  }
...
void inline rh_run(uint32_t* programBuffer, int index, void* hashIn, void* hashOut, rh_jit_program_t* program) {
